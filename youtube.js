...
var highlightWords = [];
var sessionID = '';
var remoteWindowURL = 'https://chat.example.com/overlay/';
  ...
  .on('click',
    ...
    function () {
      ...
      if (this.style.getPropertyValue('--yt-live-chat-paid-message-primary-color')) {
        data.backgroundColor = 'background-color: ' + this.style.getPropertyValue(
...
$('#primary-content')
  .append(
    '<span id="aspect-ratio-container" style="font-size: 0.7em">Aspect Ratio: <span id="aspect-ratio"></span></span>');
$('#primary-content')
  .append(
    '<span id="get-overlay-url-container"><a href="#" id="pop-out-button" class="button">Get Overlay URL</a></span>');
...
$(function () {
  ...
  //     var link =
  // 'https://chat.aaronpk.tv/redirect.php?tag='+response.tag+'&lang='+window.navigator.language+'&tz='+timezone+"&version="+version;
  // $("body").append('<div id="featured"><a href="'+link+'" target="_blank"><img src="'+response.img+'" height="32"
  // width="160"></a></span>'); } });
...
onElementInserted('.yt-live-chat-item-list-renderer#items', 'yt-live-chat-text-message-renderer', function (element) {
  ...
  console.log('onElementInserted', element);
  const userName = element?.querySelector('#author-name')?.textContent;

  console.log('userName', userName);

  if (userName) {
    const isNewChatter = !peopleList.includes(userName);
    const isStreamLabs = userName.toLowerCase().includes('streamlabs');

    console.log('isNewChatter', isNewChatter);
    console.log('isStreamLabs', isStreamLabs);

    if (isNewChatter) {
      peopleList.push(userName);
      element.classList.add('new-chatter');

      // chrome.storage.local.set({ peopleList: peopleList }).then(() => {
  ...
  let highlights = '';
  if (highlightWords) {
    highlights = chatWords.filter(value => highlightWords.includes(value.toLowerCase().replace(/[^a-z0-9]/gi, '')));
  }
